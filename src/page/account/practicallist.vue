  ...
  <div>
    ...
    <div class="view_table">
      <el-table ref="multipleTable" :data="tableData.slice((currpage - 1) * pagesize, currpage * pagesize)"
        ...
        <el-table-column prop="id" label="实收单号">
          <template slot-scope="scope">
            <span>{{scope.row.id}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="receivable_id" label="应收单号"></el-table-column>
        <el-table-column prop="type" label="财务类型"></el-table-column>
        <el-table-column prop="money" label="实收金额" show-overflow-tooltip></el-table-column>
        <el-table-column prop="time" label="实收日期" show-overflow-tooltip></el-table-column>
      </el-table>
...
<script>
  ...
  export default {
    ...
    methods: {
      getData() {
        axios.post(url + '/account/queryPractical')
          .then(response => {
